...
def start(dvmn_token, chat_id, tg_bot):
    ...
    while True:
        try:
            ...
            if checks['status'] == 'timeout':
                ...
                if work_status:
                    tg_bot.send_message(chat_id=chat_id, text=f"Урок '{work_title}' не прошел ревью. [Что от тебя хотят в этот раз]({work_link})", parse_mode='Markdown', disable_web_page_preview=True)
...
def main():
    ...
    start(dvmn_token, chat_id, tg_bot)
